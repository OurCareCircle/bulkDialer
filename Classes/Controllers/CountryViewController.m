...
@synthesize listContent, filteredListContent, sectionedListContent, savedSearchTerm, savedScopeButtonIndex, searchWasActive, chosenCountry;
...
- (void)setListContent:(NSArray *)inListContent
{
    ...
    for (NSString *countryName in listContent) {
        NSInteger section = [collation sectionForObject:countryName collationStringSelector:@selector(self)];
        [sections addObject:countryName toSubarrayAtIndex:section];
    }
...
- (id)init
{
    chosenCountry = countries[0]; //set to default country in pos zero
    //check if user has saved a country setting; if so, use that as the default
    //This is because users in Israel and UK tend to dial local test calls
    
    FirstViewController *first = [[FirstViewController alloc] init];
    [first viewDidLoad];
    NSString            *ccode = first.ccode;
    
    if ([ccode compare:@"999"] == NSOrderedSame) {
        //user has not saved settings; so, just leave it at default country
    }
    else {
        //find row# in ccodes array and set to the corresponding country
        for (int rownum = 0; rownum < (int)([FirstViewController sizeofCountries]); rownum++) {
            if ([ccodes[rownum] compare:ccode] == NSOrderedSame) {
                chosenCountry = countries[rownum];
                break;
            }
        }
    }
...
- (void)viewDidLoad
{
	...
	// create a filtered list that will contain countries for the search results table.
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (tableView == self.searchDisplayController.searchResultsTableView)
	{
        countryName = [self.filteredListContent objectAtIndex:indexPath.row];
    }
	else
	{
        countryName = [self.sectionedListContent objectAtIndexPath:indexPath];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //Remember which country was chosen and pop back to parent VC
...
- (void)filterContentForSearchText:(NSString*)searchText scope:(NSString*)scope
{
	...
	/*
	 Search the main list for countries whose type matches the scope (if selected) and whose name matches searchText; add items that match to the filtered array.
	 */
    for (NSArray *section in self.sectionedListContent) {
        for (NSString *countryName in section)
        {
            if ([scope isEqualToString:@"All"] || [countryName isEqualToString:scope])
            {
                NSComparisonResult result = [countryName compare:searchText options:(NSCaseInsensitiveSearch|NSDiacriticInsensitiveSearch) range:NSMakeRange(0, [searchText length])];
                if (result == NSOrderedSame)
                {
                    [self.filteredListContent addObject:countryName];
                }
            }
        }
    }
