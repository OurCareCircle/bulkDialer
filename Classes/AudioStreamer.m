...
cleanup:
	...
	@synchronized(self)
	{
		//013111 sleep for x seconds before quitting; else small audio will not play
		[NSThread sleepForTimeInterval:2.0];
		
		[self clearState];//clean up all state before quitting
	}

	[pool release];
}

//Do all end of view processing here
- (void)clearState {
	//
...
#ifdef TARGET_OS_IPHONE			
	//for group voicemail app, dont do this; else you cannot record
    //after plaing any audio AudioSessionSetActive(false);
#endif
...
- (void)pause
{
	@synchronized(self)
	{
		...
		else if (state == AS_PAUSED)
		{
			...
			//added on 020511; on init of plyer, we call pause to really start playing
			[self start];
		}
	}
}
